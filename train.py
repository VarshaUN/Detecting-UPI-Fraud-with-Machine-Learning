...
PLOT_PATH = os.path.join(SAVE_DIR, 'figures', 'roc_curve_logistic_100k.png')
CM_PATH = os.path.join(SAVE_DIR, 'figures', 'cm_logistic_100k.png')
...
print(f"Logistic Precision: {precision_score(y_test, y_pred):.3f}")
print(f"Logistic Recall: {recall_score(y_test, y_pred):.3f}")
print(f"Logistic F1-score: {f1_score(y_test, y_pred):.3f}")
print(f"Logistic AUC-ROC: {roc_auc_score(y_test, y_pred_proba):.3f}")

# Confusion Matrix
cm = confusion_matrix(y_test, y_pred)
plt.figure(figsize=(6, 4))
sns.heatmap(cm, annot=True, fmt='d', cmap='Blues')
plt.title('Confusion Matrix - Logistic Regression (100k)')
...
print(f"Logistic AUC: {auc_score:.3f}")
plt.plot(fpr, tpr, label=f'Logistic ROC curve (AUC = {auc_score:.3f})')
plt.plot([0, 1], [0, 1], 'k--')
plt.xlabel('False Positive Rate')
plt.ylabel('True Positive Rate')
plt.title('Logistic Regression ROC Curve (100k)')
plt.legend()
try:
